...
export class DefaultView extends LitElement {
    ...
    private renderSpotifyDisplay() {
        ...
        return html`
      <spotify-display
        .state=${this.hass.states[this.config.spotify_entity]}
        .title=${this.getSpotifyTitle()}>
      </spotify-display>
    `;
    }
    ...
    private handleShortcutsClick() {
        this.dispatchEvent(new CustomEvent('shortcuts-click'));
    }

    private handleDebugClick() {
        this.dispatchEvent(new CustomEvent('debug-click'));
    }
    ...
    private getSpotifyTitle(): string {
        const entity = this.hass.states[this.config.spotify_entity!];
        if (!entity?.attributes?.media_title || !entity?.attributes?.media_artist) {
            return '';
        }
